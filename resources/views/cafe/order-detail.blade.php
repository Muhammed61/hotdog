...
@php session()->forget('manual_print_url'); @endphp
@endif
...
<div class="row">
    <div class="col-lg-8">
        <div class="card">
            ...
            <div class="card-body">
                    ...
                    <div class="col-md-6">
                        ...
                        <p class="mb-1"><strong>Masa Oturma Süresi:</strong> 
                            <span 
                                id="tableDuration"
                                class="badge bg-info"
                                data-start-ts="{{ $order->created_at->timestamp }}"
                                data-end-ts="{{ ($order->is_paid && $order->paid_at) ? $order->paid_at->timestamp : (($order->status === 'cancelled') ? $order->updated_at->timestamp : '') }}"
                ...
                <div class="table-responsive">
                    ...
                    <table class="table table-striped">
                        ...
                        <tbody>
                            @php
                                ...
                                foreach($order->cafeOrderPayments as $payment) {
                                    if($payment->selected_items) {
                                        foreach($payment->selected_items as $pItem) {
                                            if(isset($pItem['isExtra']) && $pItem['isExtra']) {
                                                 $key = 'extra_' . $pItem['id'];
                                            } else {
                                                 $key = $pItem['id'];
                                            }
                                            
                                            if(!isset($paidQuantities[$key])) {
                                                $paidQuantities[$key] = 0;
                                            }
                                            $paidQuantities[$key] += $pItem['quantity'];
                                        }
                                    }
                                }

                                // Servis edilen ürünleri (aynı product_id + unit_price) tek satırda birleştir
                                $servedGroups = [];
                                foreach ($order->cafeOrderItems as $it) {
                                    if ($it->status === 'served') {
                                        $key = $it->product_id . '|' . number_format($it->unit_price, 2, '.', '');
                                        if (!isset($servedGroups[$key])) {
                                            $servedGroups[$key] = [
                                                'product' => $it->product,
                                                ...
                                                'total_price' => 0,
                                                'paid_quantity' => 0,
                                                'sample_item' => $it, // Varsayılan olarak ilk item
                                            ];
                                        }
                                        $servedGroups[$key]['quantity'] += $it->quantity;
                                        $servedGroups[$key]['total_price'] += $it->total_price;
                                        
                                        $thisItemPaid = $paidQuantities[$it->id] ?? 0;
                                        $servedGroups[$key]['paid_quantity'] += $thisItemPaid;
                                        
                                        // Eğer bu item henüz tamamen ödenmemişse, silme işlemi için bunu sample_item yap
                                        // Böylece "1 adet sil" dendiğinde ödenmemiş bir item silinir.
                                        if (($it->quantity - $thisItemPaid) > 0) {
                                            $servedGroups[$key]['sample_item'] = $it;
                                        }
                                    }
                                }
                            @endphp

                            {{-- Servis edilmeyen kalemleri olduğu gibi listele --}}
                            @foreach($order->cafeOrderItems->where('status', '!=', 'served') as $item)
                            @php
                                $paidQty = $paidQuantities[$item->id] ?? 0;
                                $isFullyPaid = $paidQty >= $item->quantity;
                                $isPartiallyPaid = $paidQty > 0 && !$isFullyPaid;
                            @endphp
                            <tr>
                                ...
                                <td>
                                    ...
                                    @if($isFullyPaid)
                                        <span class="badge bg-success ms-1">Ödendi</span>
                                        @php $remainQty = max(0, $item->quantity - $paidQty); @endphp
                                        <span class="badge bg-secondary ms-1">Kaldı: {{ $remainQty }}</span>
                                    @elseif($isPartiallyPaid)
                                        <span class="badge bg-warning text-dark ms-1">{{ $paidQty }} Ödendi</span>
                                        @php $remainQty = max(0, $item->quantity - $paidQty); @endphp
                                        <span class="badge bg-secondary ms-1">Kaldı: {{ $remainQty }}</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    ...
                                    {{-- Silme butonları (sadece bekliyor durumunda) --}}
                                    @if(!$order->is_paid && $order->status !== 'cancelled' && $item->status === 'pending')
                                        {{-- 1 adet sil (eğer ödenmemiş miktar varsa) --}}
                                        @if(($item->quantity - $paidQty) > 0)
                                        <form method="POST" action="{{ route('cafe.item.remove', $item) }}" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <input type="hidden" name="remove_qty" value="1">
                                            <button type="submit" class="btn btn-outline-danger btn-sm ms-1" title="1 adet sil">
                                                <i class="fas fa-minus-circle"></i>
                                            </button>
                                        </form>
                                        @endif

                                        {{-- Tamamını sil (hiç ödeme yapılmadıysa) --}}
                                        @if($paidQty == 0)
                                        <form method="POST" action="{{ route('cafe.item.remove', $item) }}" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <input type="hidden" name="remove_qty" value="{{ $item->quantity }}">
                                            <button type="submit" class="btn btn-danger btn-sm ms-1" onclick="return confirm('Bu ürünü tamamen silmek istediğinizden emin misiniz?')" title="Tümünü Sil">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        </form>
                                        @endif
                                    @elseif($order->status === 'cancelled')
                                        <span class="text-muted" title="İptal edildiği için silinemez">
                            ...
                            {{-- Servis edilen kalemleri gruplu şekilde tek satırdan göster --}}
                            @foreach($servedGroups as $group)
                            <tr>
                                <td>{{ $group['product']->name }}</td>
                                <td>{{ number_format($group['unit_price'], 2) }} ₺</td>
                                <td>
                                    <span class="badge bg-secondary">{{ $group['quantity'] }}</span>
                                    @php
                                        $gPaid = $group['paid_quantity'];
                                        $gQty = $group['quantity'];
                                        $gFullyPaid = $gPaid >= $gQty;
                                        $gPartiallyPaid = $gPaid > 0 && !$gFullyPaid;
                                    @endphp
                                    @if($gFullyPaid)
                                        <span class="badge bg-success ms-1">Ödendi</span>
                                        <span class="badge bg-secondary ms-1">Kaldı: {{ max(0, $gQty - $gPaid) }}</span>
                                    @elseif($gPartiallyPaid)
                                        <span class="badge bg-warning text-dark ms-1">{{ $gPaid }} Ödendi</span>
                                        <span class="badge bg-secondary ms-1">Kaldı: {{ max(0, $gQty - $gPaid) }}</span>
                                    @endif
                                </td>
                                ...
                                <td>
                                    @if(!$order->is_paid && $order->status !== 'cancelled')
                                        {{-- 1 adet sil --}}
                                        @if(($group['quantity'] - $group['paid_quantity']) > 0)
                                        <form method="POST" action="{{ route('cafe.item.remove', $group['sample_item']) }}" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <input type="hidden" name="remove_qty" value="1">
                                            <button type="submit" class="btn btn-outline-danger btn-sm" title="1 adet sil">
                                                <i class="fas fa-minus-circle"></i>
                                            </button>
                                        </form>
                                        @endif
                                        ...
                                        {{-- Tüm grubu sil --}}
                                        @if($group['paid_quantity'] == 0)
                                        <form method="POST" action="{{ route('cafe.served-group.remove', $order) }}" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <input type="hidden" name="product_id" value="{{ $group['product']->id }}">
                                            <input type="hidden" name="unit_price" value="{{ $group['unit_price'] }}">
                                            <button type="submit" class="btn btn-danger btn-sm ms-1" onclick="return confirm('Bu servisteki tüm {{ $group['product']->name }} ürünlerini silmek istediğinizden emin misiniz?')" title="Tümünü Sil">
                                                <i class="fas fa-trash"></i>
                                            </button>
                                        </form>
                                        @endif
                            ...
                            @if($order->cafeOrderExtras && $order->cafeOrderExtras->count() > 0)
                                ...
                                <tr class="table-success">
                                    ...
                                    <td>
                                        @if(auth()->user()->hasAnyRole(['admin', 'manager','cashier']) && !$order->is_paid && $order->status !== 'cancelled')
                                        ...
                                        @elseif($order->is_paid || $order->status === 'cancelled')
                                        <span class="text-muted" title="Ödeme alındığı için silinemez">
                                            <i class="fas fa-lock"></i>
                                        </span>
                                        @else
                ...
                <!-- Sipariş Geçmişi (Loglar) -->
                @if(auth()->user()->isAdmin())
                @if($order->cafeOrderLogs && $order->cafeOrderLogs->count() > 0)
                <div class="card mb-3">
                    <div class="card-header bg-light">
                        <h6 class="mb-0"><i class="fas fa-history me-2"></i>Sipariş Geçmişi</h6>
                    </div>
                    <div class="card-body p-0">
                        <div class="list-group list-group-flush" style="max-height: 300px; overflow-y: auto;">
                            @foreach($order->cafeOrderLogs as $log)
                                <div class="list-group-item">
                                    <div class="d-flex w-100 justify-content-between">
                                        <h6 class="mb-1 text-primary">{{ $log->user->name }} ({{ $log->user->role }})</h6>
                                        @php
                                            $isStale = $loop->first && $log->created_at->lt(now()->subHour());
                                        @endphp
                                        <small class="{{ $isStale ? 'text-danger fw-bold' : 'text-muted' }}">
                                            {{ $log->created_at->format('d.m.Y H:i:s') }}
                                            @if($isStale)
                                                <span class="badge bg-danger ms-2">1 saat+</span>
                                            @endif
                                        </small>
                                    </div>
                                    <p class="mb-1">{{ $log->message }}</p>
                                    <small class="text-muted">
                                        @if($log->action === 'item_added')
                                            <span class="badge bg-success">Ürün Eklendi</span>
                                        @elseif($log->action === 'item_removed')
                                            <span class="badge bg-danger">Ürün Silindi</span>
                                        @elseif($log->action === 'status_changed')
                                            <span class="badge bg-info">Durum Değişti</span>
                                        @elseif($log->action === 'table_status_changed')
                                            <span class="badge bg-warning text-dark">Masa Durumu</span>
                                        @elseif($log->action === 'item_status_changed')
                                            <span class="badge bg-secondary">Ürün Durumu</span>
                                        @elseif($log->action === 'order_created')
                                            <span class="badge bg-primary">Sipariş Oluşturuldu</span>
                                        @elseif($log->action === 'order_cancelled')
                                            <span class="badge bg-danger">İptal Edildi</span>
                                        @elseif($log->action === 'table_transfer')
                                            <span class="badge bg-warning text-dark">Masa Taşıma</span>
                                        @elseif($log->action === 'order_merged')
                                            <span class="badge bg-warning text-dark">Sipariş Birleştirme</span>
                                        @elseif($log->action === 'extra_added')
                                            <span class="badge bg-success">Ekstra Eklendi</span>
                                        @elseif($log->action === 'extra_removed')
                                            <span class="badge bg-danger">Ekstra Silindi</span>
                                        @else
    ...
    <div class="col-lg-4">
        ...
        @if(auth()->user()->hasAnyRole(['admin', 'manager','cashier','waiter']))
        @if($order->status === 'cancelled')
        <div class="card mb-3 mt-3">
            <div class="card-header">
                <h6 class="mb-0"><i class="fas fa-undo me-2"></i>İptali Geri Al</h6>
            </div>
            <div class="card-body">
                <form action="{{ route('cafe.order.status', $order) }}" method="POST">
                    @csrf
                    ...
                    <button type="submit" class="btn btn-warning w-100 text-dark">
                        <i class="fas fa-undo me-2"></i>İptali Geri Al ve Servis Edildi
                    </button>
                </form>
                <small class="text-muted d-block mt-2">Masa tekrar "Dolu" olur ve ürünler "Servis Edildi" durumuna geçer.</small>
            </div>
        </div>
        @endif
        <!-- Ek Sipariş Al -->
        <div class="card mb-3 mt-3">
            <div class="card-header">
                <h6 class="mb-0"><i class="fas fa-plus me-2"></i>Ek Sipariş Al</h6>
            </div>
            <div class="card-body">
                @php
                    $lastLog = $order->cafeOrderLogs->first();
                    $orderIdle = $lastLog && $lastLog->created_at->lt(now()->subHour());
                @endphp
                <a href="{{ route('cafe.table.select', $order->table) }}" class="btn {{ $order->is_paid ? 'btn-warning text-dark' : ($orderIdle ? 'btn-danger' : 'btn-success') }} w-100">
                    <i class="fas fa-shopping-cart me-2"></i>{{ $order->is_paid ? 'Bu Masadan Sipariş Al' : 'Bu Masadan Ek Sipariş Al' }}
                </a>
                <small class="d-block mt-2 {{ $orderIdle ? 'text-danger' : 'text-muted' }}">
                    @if($orderIdle)
                    ...
                    @else
                        Mevcut masaya yeni ürün eklemek için sipariş ekranına gider.
                    @endif
                </small>
            </div>
        </div>
                ...
                <div class="card-body">
                    ...
                    <div class="d-grid gap-2">
                        ...
                        <form action="{{ route('cafe.order.payment', $order) }}" method="POST" class="d-inline" id="card-payment-form">
                            ...
                            <button type="submit" class="btn btn-primary w-100" style="background-color:#8b5a2b; color:#fff; border:none;">
                                ...
                                <span id="card-button-text">
                                    ...
                                    @endif
                                        ...
                                        <!-- Kısmi Ödeme Butonu -->
                    <div class="mb-3 mt-2">
                        <a href="{{ route('cafe.order.split-payment', $order) }}" class="btn btn-warning w-100 rounded text-dark">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Masa Değiştir</h5>
            ...
            <form action="{{ route('cafe.order.transfer', $order) }}" method="POST">
                ...
                <div class="modal-body">
                    <p>Mevcut Masa: <strong>{{ $order->table->name }}</strong></p>
                    
                    <div class="mb-3">
                        <label for="target_table_id" class="form-label">Yeni Masa Seçin</label>
                        <select name="target_table_id" class="form-select" required>
                            <option value="">Seçiniz...</option>
                            @foreach(\App\Models\Table::where('id', '!=', $order->table_id)->where('status', 'available')->get() as $table)
                                <option value="{{ $table->id }}">{{ $table->name }} ({{ $table->capacity }} Kişilik)</option>
                            @endforeach
                        </select>
                        <small class="text-muted">Sadece boş masalar listelenir.</small>
                    </div>

                    <div class="mb-3">
                        <label for="transfer_reason" class="form-label">Taşıma Sebebi (Opsiyonel)</label>
                        <textarea name="transfer_reason" class="form-control" rows="2" placeholder="Örn: Müşteri cam kenarı istedi..."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">İptal</button>
                    <button type="submit" class="btn btn-warning">Masayı Taşı</button>
                </div>
            </form>
        </div>
    </div>
...
<div class="modal fade" id="mergeModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">Masa Birleştir</h5>
                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
            </div>
            <form action="{{ route('cafe.order.merge', $order) }}" method="POST">
                ...
                <div class="modal-body">
                    <div class="alert alert-info">
                        <i class="fas fa-info-circle me-2"></i>
                        Bu işlem, mevcut siparişi ({{ $order->table->name }}) başka bir dolu masa ile birleştirir.
                    </div>

                    <div class="mb-3">
                        <label for="target_order_id" class="form-label">Hedef Masa Seçin</label>
                        <select name="target_order_id" class="form-select" required>
                            <option value="">Seçiniz...</option>
                            @foreach(\App\Models\CafeOrder::where('id', '!=', $order->id)->where('status', '!=', 'cancelled')->where('is_paid', false)->get() as $targetOrder)
                                <option value="{{ $targetOrder->id }}">
                                    {{ $targetOrder->table->name }} - {{ $targetOrder->order_number }} 
                                    ({{ number_format($targetOrder->final_amount, 2) }} ₺)
                                </option>
                            @endforeach
                        </select>
                        <small class="text-muted">Sadece açık siparişi olan masalar listelenir.</small>
                    </div>

                    <div class="mb-3">
                        <label for="merge_reason" class="form-label">Birleştirme Sebebi (Opsiyonel)</label>
                        <textarea name="merge_reason" class="form-control" rows="2" placeholder="Örn: Arkadaşlar birleşti..."></textarea>
                    </div>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">İptal</button>
                    <button type="submit" class="btn btn-info">Masaları Birleştir</button>
                </div>
            </form>
        </div>
    </div>
...
<script>
    function applyDiscount(percentage) {
        ...
        let remainingAmount = {{ $order->remaining_amount }};
        let discountAmount = remainingAmount * (percentage / 100);
        let finalAmount = remainingAmount - discountAmount;
        
        // Bilgi göster
        document.getElementById('discount-info').style.display = 'block';
        document.getElementById('discount-text').innerHTML = 
            '<strong>%' + percentage + '</strong> indirim uygulanacak.<br>' +
            'İndirim Tutarı: ' + discountAmount.toFixed(2) + ' ₺<br>' +
            'Yeni Tutar: <strong>' + finalAmount.toFixed(2) + ' ₺</strong>';
            
        // Butonları güncelle
        let isPartial = {{ $order->is_partially_paid ? 'true' : 'false' }};
        if(isPartial) {
            document.getElementById('cash-button-text').innerText = 'Kalan Tutarı Nakit Öde (' + finalAmount.toFixed(2) + ' ₺)';
            document.getElementById('card-button-text').innerText = 'Kalan Tutarı Kart ile Öde (' + finalAmount.toFixed(2) + ' ₺)';
        } else {
            document.getElementById('cash-button-text').innerText = 'Nakit Ödeme (' + finalAmount.toFixed(2) + ' ₺)';
            document.getElementById('card-button-text').innerText = 'Kredi Kartı ile Ödeme (' + finalAmount.toFixed(2) + ' ₺)';
        }
    }
    ...
    function clearDiscount() {
        ...
        if(isPartial) {
            document.getElementById('cash-button-text').innerText = 'Kalan Tutarı Nakit Öde (' + remainingAmount.toFixed(2) + ' ₺)';
            document.getElementById('card-button-text').innerText = 'Kalan Tutarı Kart ile Öde (' + remainingAmount.toFixed(2) + ' ₺)';
        } else {
            document.getElementById('cash-button-text').innerText = 'Nakit Ödeme';
            document.getElementById('card-button-text').innerText = 'Kredi Kartı ile Ödeme';
        }
    }
</script>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    setTimeout(function() {
        if (typeof qz === 'undefined') {
            ...
            errorDiv.innerHTML = "<i class='fas fa-exclamation-triangle me-2'></i><strong>QZ Tray Bulunamadı!</strong> QZ Tray programını başlatın ve sayfayı yenileyin.<button type='button' class='btn-close' data-bs-dismiss='alert'></button>";
...
@php session()->forget('qz_direct_print'); @endphp
@endif
