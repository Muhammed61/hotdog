...
class CafeController extends Controller
{
    ...
    {
        $order->load(['table', 'user', 'cafeOrderItems.product', 'cafeOrderNotes.user', 'cafeOrderExtras']);
    ...
    {
        try {
            // Mevcut siparişin masası hariç, dolu masaları getir
            $occupiedTables = Table::whereHas('cafeOrders', function($query) {
                $query->where('is_paid', false);
            })
            ->where('id', '!=', $order->table_id)
            ->get()
            ->map(function($table) {
                $orderCount = $table->cafeOrders()->where('is_paid', false)->count();
                return [
                    'id' => $table->id,
                    'name' => $table->name,
    ...
    public function mergeOrder(Request $request, CafeOrder $order)
    {
        ...
        try {
            ...
            $targetOrder = CafeOrder::where('table_id', $targetTable->id)
                                   ->where('is_paid', false)
                                   ->first();

            if (!$targetOrder) {
                return back()->with('error', 'Hedef masada aktif sipariş bulunamadı!');
            }

            // Mevcut siparişin ürünlerini hedef siparişe taşı
            foreach ($order->cafeOrderItems as $item) {
                // Aynı üründen hedef siparişte var mı kontrol et
                ...
                if ($existingItem) {
                    // Varsa miktarı artır ve total_price'ı güncelle
                    ...
                    // Yoksa yeni item olarak ekle
                    $item->update(['cafe_order_id' => $targetOrder->id]);
                }
            }

            // Ekstra ücretleri de taşı
            foreach ($order->cafeOrderExtras as $extra) {
                $extra->update(['cafe_order_id' => $targetOrder->id]);
            }

            // Hedef siparişin toplam tutarını güncelle
            $targetOrder->total_amount = $targetOrder->cafeOrderItems->sum('total_price') + $targetOrder->cafeOrderExtras->sum('amount');
            $targetOrder->save();
    ...
    public function removeOrderItem(CafeOrderItem $item)
    {
        ...
        // Servis kontrolünü kaldırdık - artık her durumda silinebilir
     ...
     */
    private function manualPrintReceipt($order)
    {
        try {
            // Kompakt fiş içeriği oluştur
            $receiptContent = $this->generateCompactReceipt($order);
            
            // Windows ortamı kontrolü
            if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN') {
                // Windows'ta Notepad ile yazdır
                $this->printWithNotepad($order, $receiptContent);
            } else {
                // Linux/Unix sunucuda tarayıcı ile yazdır
